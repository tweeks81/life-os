...
export default function ConnectionsPanel({
  ...
  const refresh = async () => {
    const [{ data: linkedRaw }, { data: sentRaw }, { data: receivedRaw }] = await Promise.all([
      (supabase as any)
    ...
    const allUserIds = [
      ...(linkedRaw ?? []).map((l: any) => l.linked_user_id),
      ...(sentRaw ?? []).map((r: any) => r.to_user_id),
    ...
    const findProfile = (id: string) => (profiles ?? []).find((p: any) => p.id === id) ?? null

    setLinked((linkedRaw ?? []).map((l: any) => ({ ...l, profile: findProfile(l.linked_user_id) })))
    setSent((sentRaw ?? []).map((r: any) => ({ ...r, to_profile: findProfile(r.to_user_id) })))
    setReceived((receivedRaw ?? []).map((r: any) => ({ ...r, from_profile: findProfile(r.from_user_id) })))
  }
  ...
  return (
    <div className="connections-panel card">
        ...
        <div className="panel-section">
          ...
          {pendingReceived.map(req => (
            <div key={req.id} className="request-row">
              <Avatar url={(req as any).from_profile?.avatar_url} name={(req as any).from_profile?.full_name} size={36} />
              <div className="request-info">
                <span className="request-name">{(req as any).from_profile?.full_name ?? (req as any).from_profile?.email}</span>
                <span className="request-email">{(req as any).from_profile?.email}</span>
              </div>
      ...
      <div className="panel-section">
        <div className="section-label">Connected ({linked.length})</div>
        {linked.length === 0 ? (
          <p className="empty-msg">No connections yet. Send a request above.</p>
        ) : (
          linked.map(link => (
            <div key={link.id} className="linked-row">
              <Avatar url={(link as any).profile?.avatar_url} name={(link as any).profile?.full_name} size={36} />
              <div className="linked-info">
                <span className="linked-name">{(link as any).profile?.full_name ?? (link as any).profile?.email}</span>
                <span className="linked-email">{(link as any).profile?.email}</span>
              </div>
              <button className="unlink-btn" onClick={() => setConfirmUnlink(link)}>Remove</button>
            </div>
          ))
        )}
      </div>
          ...
          {sent.filter(r => r.status === 'pending').map(req => (
            <div key={req.id} className="sent-row">
              <div className="sent-info">
                <span className="sent-name">{(req as any).to_profile?.full_name ?? (req as any).to_profile?.email}</span>
                <span className="sent-email">{(req as any).to_profile?.email}</span>
              </div>
        ...
        <div className="confirm-backdrop" onClick={() => setConfirmUnlink(null)}>
          <div className="confirm-box card" onClick={e => e.stopPropagation()}>
            ...
            <p className="confirm-msg">
              This will remove your connection with <strong>{(confirmUnlink as any).profile?.full_name ?? (confirmUnlink as any).profile?.email}</strong>. They will no longer appear as a linked contact.
            </p>
      ...
      <style>{`
        .connections-panel { padding: 1.75rem 2rem; display: flex; flex-direction: column; gap: 0; }
        .panel-header { margin-bottom: 1.25rem; }
        .panel-title { font-size: 1.125rem; font-weight: 600; margin-bottom: 0.25rem; }
        .panel-desc { font-size: 0.875rem; color: var(--text-muted); }
        .panel-section { padding: 1rem 0; border-top: 1px solid var(--border-light); display: flex; flex-direction: column; gap: 0.625rem; }
        .section-label { font-size: 0.7rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text-muted); margin-bottom: 0.125rem; }
        .send-row { display: flex; gap: 0.5rem; }
        .send-btn { font-size: 0.875rem; padding: 0.5rem 1rem; white-space: nowrap; flex-shrink: 0; }
        .feedback-error { font-size: 0.8125rem; color: #dc2626; }
        .feedback-success { font-size: 0.8125rem; color: #16a34a; }
        .request-row, .linked-row, .sent-row { display: flex; align-items: center; gap: 0.75rem; padding: 0.625rem 0.75rem; background: var(--cream); border-radius: 10px; border: 1px solid var(--border-light); }
        .request-info, .linked-info, .sent-info { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 0.1rem; }
        .request-name, .linked-name, .sent-name { font-size: 0.875rem; font-weight: 600; color: var(--text-primary); }
        .request-email, .linked-email, .sent-email { font-size: 0.75rem; color: var(--text-muted); }
        .request-actions { display: flex; gap: 0.375rem; flex-shrink: 0; }
        .accept-btn { font-size: 0.8125rem; padding: 0.35rem 0.75rem; }
        .reject-btn { font-size: 0.8125rem; padding: 0.35rem 0.75rem; }
        .unlink-btn { font-size: 0.8rem; color: var(--text-muted); background: none; border: 1px solid var(--border); border-radius: 6px; padding: 0.3rem 0.625rem; cursor: pointer; font-family: var(--font-body); transition: all 0.15s; flex-shrink: 0; }
        .unlink-btn:hover { background: #fef2f2; border-color: #fecaca; color: #dc2626; }
        .status-pill { font-size: 0.72rem; font-weight: 600; padding: 0.2rem 0.5rem; border-radius: 100px; flex-shrink: 0; }
        ...
        .empty-msg { font-size: 0.875rem; color: var(--text-muted); font-style: italic; }
        .confirm-backdrop { position: fixed; inset: 0; background: rgba(44,31,20,0.35); z-index: 300; display: flex; align-items: center; justify-content: center; padding: 1rem; }
        .confirm-box { max-width: 400px; width: 100%; padding: 1.5rem; display: flex; flex-direction: column; gap: 0.875rem; }
        .confirm-title { font-size: 1rem; font-weight: 600; }
        .confirm-msg { font-size: 0.875rem; color: var(--text-secondary); line-height: 1.6; }
        .confirm-actions { display: flex; justify-content: flex-end; gap: 0.5rem; padding-top: 0.5rem; border-top: 1px solid var(--border-light); }
        .btn-danger { display: inline-flex; align-items: center; justify-content: center; padding: 0.625rem 1.25rem; background: #dc2626; color: white; border: none; border-radius: var(--radius-sm); font-size: 0.875rem; font-weight: 500; font-family: var(--font-body); cursor: pointer; transition: all 0.15s; }
