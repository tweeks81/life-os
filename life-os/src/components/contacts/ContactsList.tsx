...
export default function ContactsList({
  ...
  const grouped = useMemo(() => {
    ...
    for (const contact of filtered) {
      const letter = contact.last_name[0]?.toUpperCase() ?? '#'
  ...
  return (
    <div className="contacts-list">
      <div className="list-header">
        <h2 className="list-title">Contacts <span className="list-count">{contacts.length}</span></h2>
          ...
          grouped.map(([letter, groupContacts]) => (
            <div key={letter} className="alpha-group">
              ...
              {groupContacts.map(contact => (
                <button
                  key={contact.id}
                  className={`contact-row ${selectedContactId === contact.id ? 'selected' : ''}`}
                ...
                >
                  ...
                  <div className="contact-info">
                    ...
                    {contact.email && (
                      <span className="contact-sub">{contact.email}</span>
                    )}
                    {!contact.email && contact.phone_mobile && (
                      <span className="contact-sub">{contact.phone_mobile}</span>
                    )}
                  </div>
                  {contact.user_id !== userId && (
                    <span className="shared-pill">👥</span>
                  )}
                </button>
              ))}
            </div>
