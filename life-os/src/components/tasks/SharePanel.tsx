...
export default function SharePanel({
  ...
  entityId: string
  entityType: 'task' | 'project' | 'contact' | 'calendar_event' | 'property' | 'vehicle'
  ownerId: string
  userId: string
  shares: ShareRecord[]
  ...
  const table = entityType === 'task' ? 'task_shares'
    ...
    : entityType === 'property' ? 'property_shares'
    : 'vehicle_shares'
  ...
  useEffect(() => {
    if (!isOwner) { setLoadingLinked(false); return }
    ;(async () => {
      ...
      if (linkedIds.length === 0) { setLoadingLinked(false); return }
  ...
  const handleShare = async () => {
    ...
    for (const email of Array.from(selected)) {
      ...
      const { error: err } = await (supabase.from(table) as any).insert({
        [idField]: entityId,
        owner_id: ownerId,
        shared_with_email: email,
        shared_with_id: person?.linked_user_id ?? null,
      })
      if (err) setError('Failed to share with one or more people.')
    }
  ...
  const getDisplayName = (email: string) => {
    const person = linkedPeople.find(p => p.email.toLowerCase() === email.toLowerCase())
    return person?.full_name ?? email
  }
    ...
    <div className="share-panel">
        ...
        <ul className="share-list">
          {shares.map(s => (
            <li key={s.id} className="share-item">
              <span className="share-avatar">{getDisplayName(s.shared_with_email)[0].toUpperCase()}</span>
              <span className="share-name">{getDisplayName(s.shared_with_email)}</span>
              {isOwner && (
                <button
      ...
      {isOwner && !loadingLinked && (
        <>
          ...
          {!showPicker && available.length === 0 && linkedPeople.length === 0 && (
            <p className="share-hint">
              Connect with people on your <strong>Profile</strong> page to share with them.
            </p>
          )}

          {!showPicker && available.length === 0 && linkedPeople.length > 0 && (
            <p className="share-hint">Shared with all your connections.</p>
          )}

          {showPicker && (
            <div className="picker">
              <p className="picker-label">Select connections to share with:</p>
              <div className="picker-list">
                {available.map(p => {
                  ...
                  return (
                    ...
                    >
                      <div className="picker-check">{isSelected ? '✓' : ''}</div>
              ...
              <div className="picker-footer">
                <button
                  className="btn-secondary picker-cancel"
                ...
                >
                  {adding ? 'Sharing…' : selected.size === 0 ? 'Select people' : `Share with ${selected.size} ${selected.size === 1 ? 'person' : 'people'}`}
                </button>
              </div>
            </div>
          )}
      ...
      <style>{`
        ...
        .share-name { flex: 1; font-size: 0.8125rem; color: var(--text-secondary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-weight: 500; }
