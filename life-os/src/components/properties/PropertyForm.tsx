...
export default function PropertyForm({
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error: uploadError } = await supabase.storage.from('property-photos').upload(filePath, file, { upsert: true })
    if (uploadError) { setError('Failed to upload photo.'); setUploading(false); return }
  ...
  const handleSave = async () => {
    ...
    if (isEdit) {
      const { data, error: err } = await supabase.from('properties').update(payload).eq('id', property.id).select().single()
      if (err) { setError('Failed to save.'); setSaving(false); return }
      onSaved(data as Property)
    } else {
      const { data, error: err } = await supabase.from('properties').insert({ ...payload, user_id: userId }).select().single()
      if (err) { setError('Failed to save.'); setSaving(false); return }
      onSaved(data as Property)
    }
  }
      ...
      <div className="modal-box card">
        ...
        <div className="modal-body">
          <div className="photo-section">
            {photoUrl ? (
              <div className="photo-preview">
                <Image src={photoUrl} alt="Property" width={200} height={140} style={{ width: '100%', height: '140px', objectFit: 'cover', borderRadius: '8px' }} unoptimized />
                <button className="photo-remove" onClick={() => setPhotoUrl('')} type="button">Remove photo</button>
              </div>
            ) : (
              <button className="photo-upload-btn" onClick={() => fileInputRef.current?.click()} disabled={uploading} type="button">
          ...
          <div className="field-group">
            ...
            <input className="input-field" placeholder='e.g. "Home", "Holiday cottage"' value={name} onChange={e => setName(e.target.value)} autoFocus />
          </div>
          ...
          <div className="field-group">
            ...
            <input className="input-field" type="number" placeholder="e.g. 1985" min="1000" max={new Date().getFullYear()} value={yearBuilt} onChange={e => setYearBuilt(e.target.value)} style={{ maxWidth: '160px' }} />
          </div>

          <div className="field-group">
            <label className="checkbox-label">
              <input type="checkbox" checked={isPrimary} onChange={e => setIsPrimary(e.target.checked)} className="checkbox-input" />
              <span>This is my primary residence</span>
            </label>
            {isPrimary && <p className="field-hint">Any other property currently set as primary will be unset automatically.</p>}
          </div>
          ...
          <div className="field-group">
            ...
            <textarea className="input-field" rows={3} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any additional notes…" style={{ resize: 'vertical' }} />
          </div>
